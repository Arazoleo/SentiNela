...
import { Send, WifiOff, Activity, ArrowDown } from "lucide-react";
...
export default function ChatWindow() {
  ...
  const {
    messages, isTyping, syndrome, nearestClinics, isConnected,
    addMessage, setTyping, setSyndrome, setNearestClinics, setSessionId, setConnected,
  } = useChatStore();
  ...
  useEffect(() => {
    if (!token) return;
    const ws = new SentinelaWS(token);
    wsRef.current = ws;
    ...
    const unsub = ws.onMessage((msg) => {
      ...
      if (msg.type === "connected") {
        ...
        addMessage({ id: crypto.randomUUID(), role: "assistant", content: msg.message!, timestamp: new Date() });
        return;
      }
      if (msg.type === "typing") { setTyping(true); return; }
      if (msg.type === "message" || msg.type === "classification") {
        if (msg.message) addMessage({ id: crypto.randomUUID(), role: "assistant", content: msg.message, timestamp: new Date() });
    ...
    ws.connect();
    return () => { unsub(); ws.disconnect(); setConnected(false); };
  }, [token]);
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: "#020e07" }}>
      ...
      <div
        ...
        style={{
          borderBottom: "1px solid rgba(13,51,32,0.6)",
          background: "rgba(3,14,8,0.8)",
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <span
              ...
              style={{
                borderColor: "#020e07",
                background: isConnected ? "#00ff87" : "#2a5e3a",
                boxShadow: isConnected ? "0 0 6px #00ff87" : "none",
                transition: "all 0.4s",
              }}
            />
          </div>
      ...
      <div
        ...
        style={{
          background: "radial-gradient(ellipse 80% 50% at 50% 0%, rgba(0,214,122,0.025) 0%, transparent 60%), #020e07",
        }}
      >
      ...
      <div
        ...
        style={{ borderTop: "1px solid rgba(13,51,32,0.5)" }}
      >
        <div
          ...
          onFocusCapture={e => {
            (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(0,214,122,0.3)";
            (e.currentTarget as HTMLDivElement).style.boxShadow = "0 0 0 3px rgba(0,214,122,0.06)";
          }}
          onBlurCapture={e => {
            (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(13,51,32,0.8)";
          ...
          <motion.button
            ...
            style={{
              background: input.trim() && isConnected
                ? "linear-gradient(135deg,#00d67a,#00a85c)"
                : "rgba(13,51,32,0.5)",
              boxShadow: input.trim() && isConnected ? "0 0 12px rgba(0,214,122,0.3)" : "none",
            }}
          >
